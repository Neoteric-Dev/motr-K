...
public class MotrBlockTagProvider extends BlockTagsProvider {
    ...
    private Block[] getAllSandBlocks() {
        return MotrBlocks.REGISTERED_STABLE_SANDS.values()
                .stream()
                .map(blockInfo -> blockInfo.block().get())
                .toArray(Block[]::new);
    }

    private Block[] getAllConcretePowderBlocks() {
        return MotrBlocks.REGISTERED_STABLE_CONCRETE_POWDERS.values()
                .stream()
                .map(blockInfo -> blockInfo.block().get())
                .toArray(Block[]::new);
    }
    ...
    private Block[] getAllAnvilBlocks() {
        return MotrBlocks.REGISTERED_STABLE_ANVILS.values()
                .stream()
                .map(blockInfo -> blockInfo.block().get())
                .toArray(Block[]::new);
    }
