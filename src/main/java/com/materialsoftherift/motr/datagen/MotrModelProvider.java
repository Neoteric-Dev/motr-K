...
public class MotrModelProvider extends ModelProvider {
    ...
    protected void registerModels(BlockModelGenerators blockModels, @NotNull ItemModelGenerators itemModels) {
        ...
        MotrBlocks.REGISTERED_STABLE_SANDS.forEach((texture, blockInfo) ->
                simpleVanillaModelRedirect(blockModels, blockInfo.block().get(), texture));
        MotrBlocks.REGISTERED_STABLE_CONCRETE_POWDERS.forEach((texture, blockInfo) ->
                simpleVanillaModelRedirect(blockModels, blockInfo.block().get(), texture));
        MotrBlocks.REGISTERED_STABLE_ANVILS.forEach((texture, blockInfo) ->
                anvilVanillaModelRedirect(blockModels, blockInfo.block().get(), texture));
    ...
    private void anvilVanillaModelRedirect(BlockModelGenerators blockModels, Block motrBlock, String textureName) {
        TextureMapping mapping = TextureMapping
                .cube(ResourceLocation.withDefaultNamespace("block/anvil"))
