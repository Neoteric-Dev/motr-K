...
import com.materialsoftherift.motr.item.GlowingBlockItem;
...
public class MotrItems {
    ...
    public static <T extends Block> void registerSimpleBlockItem(String id, DeferredBlock<T> block) {
        DeferredItem<BlockItem> simpleBlockItem = ITEMS.register(id, () -> new GlowingBlockItem(block.get(),
                new Item.Properties().setId(ResourceKey.create(Registries.ITEM, block.getId())).useBlockDescriptionPrefix()));

        BLOCK_ITEMS.add(simpleBlockItem);
    }

    public static <T extends Block> void registerSimpleDevBlockItem(String id, DeferredBlock<T> block) {
        DeferredItem<BlockItem> simpleBlockItem = ITEMS.register(id, () -> new GlowingBlockItem(block.get(),
                new Item.Properties().setId(ResourceKey.create(Registries.ITEM, block.getId())).useBlockDescriptionPrefix()));
    ...
    public static <T extends Block> void registerStableBlockItem(String id, DeferredBlock<T> block) {
        DeferredItem<BlockItem> simpleBlockItem = ITEMS.register(id, () -> new StableBlockItem(block.get(),
                new Item.Properties().setId(ResourceKey.create(Registries.ITEM, block.getId())).useBlockDescriptionPrefix()));
